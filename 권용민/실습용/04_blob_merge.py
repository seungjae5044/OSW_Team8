...
threshold_red = (214,239,134,194,25,58) # 빨간색 임계값 설정
    ...
    while blobs:
        ...
        for merged_blob in merged_blobs:
            ...
            if dx <= distance_threshold and dy <= distance_threshold:
                x1 = "(min or max)"(current_blob.x, merged_blob.x)
                y1 = "(min or max)"(current_blob.y, merged_blob.y)
                x2 = "(min or max)"(current_blob.x + current_blob.w, merged_blob.x + merged_blob.w)
                y2 = "(min or max)"(current_blob.y + current_blob.h, merged_blob.y + merged_blob.h)
